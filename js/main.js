...
$('[type="email"]').blur(function() {
    if($(this).val() != '') {
        ...
        if(pattern.test($(this).val())){
            ...
            if($(this).next('.emailError').length > 0){
                $('.emailError').fadeIn();    
            }else {
                $(this).after('<div style="display:none" class="emailError">Неверно введен Email!</div>');
                $('.emailError').fadeIn();
            }
        }
    } else {
        $(this).addClass('invalidInput').attr('placeholder', 'Вы не ввели email');
    }
});
$('[type="tel"]').blur( function() {
    if($(this).val() == '') {
            $(this).addClass('invalidInput').attr('placeholder', 'Вы не ввели номер телефона!');
    } else {
        $(this).attr('placeholder', 'Ваш номер телефона +38(0**) *** ** **');
    }
});
...
$('input').focus(function () {
    ...
    if ($(this).next('.emailError').length > 0) $('.emailError').fadeOut();
});
...
$('form').submit(function(e){
    ...
    if ($(this).find('.invalidInput').length == 0) {
        ...
        $(this).find('.invalidInput').css('opacity', .2).animate({
            opacity: 1
            }, 1000, function() {
